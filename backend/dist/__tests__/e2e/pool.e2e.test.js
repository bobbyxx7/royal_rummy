...
describe('E2E/Pool Rummy - deterministic via HTTP advance', () => {
    ...
    test('pool progresses and eliminates to single winner', async () => {
        ...
        await new Promise((resolve, reject) => {
            ...
            catch { } ; reject(new Error('E2E pool timed out')); }, 25000);
            let tableId;
            let started = false;
            const maybe = async () => {
                if (!started || !tableId)
                ...
                for (let i = 0; i < 100; i++) {
                    ...
                    const anyGame = (g.body?.data || []).find((gg) => gg.tableId === tableId);
                    if (!anyGame) {
                        await new Promise(r => setTimeout(r, 100));
                        continue;
                    }
                    const adv = await (0, supertest_1.default)(app).post('/api/test/pool/advance')
                    ...
                    const eliminatedResp = new Set((adv.body?.data?.eliminated || []));
                    ...
                    const pool = (fs.body?.data?.pool || []);
                    const row = pool.find((d) => d.tableId === tableId);
                    const eliminated = new Set((row?.eliminated || []));
