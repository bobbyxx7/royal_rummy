...
describe('Wallet routes', () => {
    ...
    test('GET /balance requires user_id', async () => {
        const app = makeApp();
        const res = await (0, supertest_1.default)(app).get('/api/wallet/balance');
        expect(res.status).toBe(400);
    });
    test('GET /balance returns value', async () => {
        const app = makeApp();
        const res = await (0, supertest_1.default)(app).get('/api/wallet/balance').set('x-user-id', 'u1').set('x-user-token', 't');
