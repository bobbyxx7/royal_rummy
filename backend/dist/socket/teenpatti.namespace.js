...
exports.teenPattiNamespace = teenPattiNamespace;
const auth_1 = require("../auth");
const logger_1 = require("../logger");
const errors_1 = require("../errors");
const teenpatti_state_1 = require("./teenpatti.state");
const teenpatti_bots_1 = require("./teenpatti.bots");
const teenpatti_schemas_1 = require("./teenpatti.schemas");
// Constants
const TEEN_PATTI_ROOM = (tableId) => `teenpatti:${tableId}`;
...
function teenPattiNamespace(io) {
    ...
    try {
        ...
        defaultBootValues.forEach(bootValue => {
            (0, teenpatti_state_1.createTeenPattiTable)(bootValue, 2); // 2-player tables by default
            (0, teenpatti_state_1.createTeenPattiTable)(bootValue, 6); // 6-player tables by default
        });
    }
    catch (error) {
        (0, logger_1.logSocket)('error', { error: 'Failed to seed default tables', details: error });
    }
    nsp.on('connection', async (socket) => {
        (0, logger_1.logSocket)('connected', { socketId: socket.id, namespace: '/teenpatti' });
        // Create session on connection
        const { userId, token } = socket.handshake.query;
        if (userId && token) {
            (0, teenpatti_state_1.createTeenPattiSession)(socket.id, String(userId), String(token));
        }
        ...
        socket.use(async (packet, next) => {
            try {
                ...
                const session = (0, teenpatti_state_1.findTeenPattiSessionBySocket)(socket.id);
                if (!session) {
        ...
        socket.on('get-table', async (payload) => {
            ...
            try {
                const parsed = teenpatti_schemas_1.getTeenPattiTableSchema.safeParse(payload || {});
                if (!parsed.success) {
                    socket.emit('get-table', {
                        code: errors_1.ErrorCodes.INVALID_REQUEST,
                ...
                if (!session) {
                    socket.emit('get-table', {
                        code: errors_1.ErrorCodes.UNAUTHORIZED,
                ...
                const table = (0, teenpatti_state_1.createTeenPattiTable)(String(boot_value ?? '100'), Number(no_of_players ?? 2));
                // Join the table room
                ...
                socket.emit('get-table', {
                    code: errors_1.ErrorCodes.SERVER_ERROR,
        ...
        socket.on('join-table', async (payload) => {
            ...
            try {
                const parsed = teenpatti_schemas_1.joinTeenPattiTableSchema.safeParse(payload || {});
                if (!parsed.success) {
                    socket.emit('join-table', {
                        code: errors_1.ErrorCodes.INVALID_REQUEST,
                ...
                const result = (0, teenpatti_state_1.joinTeenPattiTable)(table_id, session.userId);
                if (!result.success) {
                    socket.emit('join-table', {
                        code: errors_1.ErrorCodes.INVALID_REQUEST,
                ...
                if (result.currentPlayers >= MIN_PLAYERS_TO_START && result.canStart) {
                    ...
                    setTimeout(() => {
                        (0, teenpatti_state_1.startTeenPattiGame)(table_id);
        ...
        socket.on('start-game', async (payload) => {
            ...
            try {
                const parsed = teenpatti_schemas_1.startTeenPattiGameSchema.safeParse(payload || {});
                if (!parsed.success) {
                    socket.emit('start-game', {
                        code: errors_1.ErrorCodes.INVALID_REQUEST,
                ...
                // Start the game
                const result = (0, teenpatti_state_1.startTeenPattiGame)(table_id);
                if (!result.success) {
                    socket.emit('start-game', {
                        code: errors_1.ErrorCodes.INVALID_REQUEST,
                        message: result.message
                ...
                setTimeout(() => {
                    const game = (0, teenpatti_state_1.getTeenPattiGame)(result.gameId);
                    if (game) {
                        // Notify each player of their cards privately
                        game.players.forEach(player => {
                            if (!player.isBot && player.cards) {
                                const playerSocket = teenpatti_state_1.teenPattiUserIdToSocket.get(player.userId);
                                if (playerSocket) {
                        ...
                        if (currentPlayer && !currentPlayer.isBot) {
                            (0, teenpatti_state_1.startTurnTimer)(game.id, currentPlayer.userId);
                        }
        ...
        socket.on('bet', async (payload) => {
            ...
            try {
                const parsed = teenpatti_schemas_1.teenPattiBetSchema.safeParse(payload || {});
                if (!parsed.success) {
                    socket.emit('bet', {
                        code: errors_1.ErrorCodes.INVALID_REQUEST,
                ...
                const result = (0, teenpatti_state_1.processBetAction)(session.tableId, session.userId, action, amount, target_player_id);
                if (!result.success) {
                    socket.emit('bet', {
                        code: errors_1.ErrorCodes.INVALID_REQUEST,
                        message: result.message
                ...
                const table = teenpatti_state_1.teenPattiTables.get(session.tableId);
                if (table?.currentGameId && result.nextTurn !== undefined) {
                    const game = (0, teenpatti_state_1.getTeenPattiGame)(table.currentGameId);
                    if (game) {
                        const nextPlayer = game.players[result.nextTurn];
                        if (nextPlayer && !nextPlayer.isBot && !nextPlayer.hasFolded) {
                            (0, teenpatti_state_1.startTurnTimer)(game.id, nextPlayer.userId);
                        }
                    }
                }
        ...
        socket.on('show', async (payload) => {
            ...
            try {
                ...
                const result = (0, teenpatti_state_1.processShowAction)(session.tableId, session.userId, target_player_id);
                if (!result.success) {
                    socket.emit('show', {
                        code: errors_1.ErrorCodes.INVALID_REQUEST,
                        message: result.message
        ...
        socket.on('pack', async () => {
            try {
                ...
                const result = (0, teenpatti_state_1.processPackAction)(session.tableId, session.userId);
                if (!result.success) {
                    socket.emit('pack', {
                        code: errors_1.ErrorCodes.INVALID_REQUEST,
                        message: result.message
        ...
        socket.on('get-public-tables', async (payload) => {
            ...
            try {
                ...
                const tables = (0, teenpatti_state_1.getPublicTables)(boot_value, no_of_players);
        ...
        socket.on('quick-join', async (payload) => {
            ...
            try {
                ...
                const table = (0, teenpatti_state_1.findBestTable)(boot_value, no_of_players);
                if (!table) {
                    socket.emit('quick-join', {
                        code: errors_1.ErrorCodes.SERVER_ERROR,
                ...
                // Join the table
                const result = (0, teenpatti_state_1.joinTeenPattiTable)(table.id, session.userId);
                if (!result.success) {
                    socket.emit('quick-join', {
                        code: errors_1.ErrorCodes.INVALID_REQUEST,
                        message: result.message
        ...
        socket.on('disconnect', () => {
            (0, logger_1.logSocket)('disconnected', { socketId: socket.id, namespace: '/teenpatti' });
    ...
    async function processBotTurns(tableId, namespace) {
        try {
            const table = teenpatti_state_1.teenPattiTables.get(tableId);
            ...
            const currentPlayer = game.players[game.currentTurn];
            if (!currentPlayer || !currentPlayer.isBot || currentPlayer.hasFolded)
                return;
            // Add small delay for realistic bot behavior
            setTimeout(async () => {
                const botConfig = teenpatti_bots_1.DEFAULT_BOT_CONFIGS[teenpatti_bots_1.BotDifficulty.MEDIUM];
                const decision = (0, teenpatti_bots_1.makeBotDecision)(currentPlayer, game, botConfig);
                (0, logger_1.logSocket)('info', {
                    event: 'bot-action',
                ...
                switch (decision.action) {
                    case 'call':
                    case 'raise':
                        const betResult = (0, teenpatti_state_1.processBetAction)(tableId, currentPlayer.userId, decision.action, decision.amount);
                        ...
                        break;
                    case 'pack':
                        const packResult = (0, teenpatti_state_1.processPackAction)(tableId, currentPlayer.userId);
                    ...
                    case 'show':
                        if (decision.targetPlayerId) {
                            const showResult = (0, teenpatti_state_1.processShowAction)(tableId, currentPlayer.userId, decision.targetPlayerId);
