...
import { CardCode, validateTeenPattiCards, getTeenPattiWinner, calculateTeenPattiWinnings } from './teenpatti.rules';
import { TeenPattiTableInfo, TeenPattiPlayerInfo, TeenPattiGameState } from './teenpatti.schemas';

// In-memory state storage
export const teenPattiSessions = new Map<string, TeenPattiUserSession>();
export const teenPattiTables = new Map<string, TeenPattiTable>();
export const teenPattiGames = new Map<string, TeenPattiGame>();
export const teenPattiUserIdToSocket = new Map<string, string>();

// Types
export type TeenPattiUserSession = {
  socketId: string;
  userId: string;
  token: string;
  tableId?: string;
...
export type TeenPattiTable = {
  ...
  waitingPlayers: string[]; // Players waiting to join
};

export type TeenPattiPlayer = {
  userId: string;
  seatNo: number;
  isBot: boolean;
  isConnected: boolean;
  ...
  totalBet: number;
  cards?: CardCode[];
  isSeen: boolean; // Whether player has seen their cards
  lastSeen?: number;
};

export type TeenPattiGame = {
  id: string;
  tableId: string;
  phase: 'waiting' | 'dealing' | 'blind-betting' | 'seen-betting' | 'showdown' | 'finished';
  players: TeenPattiPlayer[];
  currentTurn: number;
  pot: number;
  deck: CardCode[];
...
// Session management
export function createTeenPattiSession(socketId: string, userId: string, token: string): TeenPattiUserSession {
  const session: TeenPattiUserSession = {
    socketId,
    userId,
    token,
    isConnected: true,
  ...
  teenPattiSessions.set(socketId, session);
  teenPattiUserIdToSocket.set(userId, socketId);
...
export function findTeenPattiSessionBySocket(socketId: string): TeenPattiUserSession | undefined {
  return teenPattiSessions.get(socketId);
}

export function findTeenPattiSessionByUserId(userId: string): TeenPattiUserSession | undefined {
  const socketId = teenPattiUserIdToSocket.get(userId);
  return socketId ? teenPattiSessions.get(socketId) : undefined;
}

export function removeTeenPattiSession(socketId: string): void {
  const session = teenPattiSessions.get(socketId);
  if (session) {
    teenPattiUserIdToSocket.delete(session.userId);
    teenPattiSessions.delete(socketId);
...
// Table management
export function createTeenPattiTable(bootValue: string, noOfPlayers: number): TeenPattiTable {
  // Check if table already exists with same boot value and player count
  for (const table of teenPattiTables.values()) {
    if (table.bootValue === bootValue && table.noOfPlayers === noOfPlayers && table.status === 'waiting') {
      return table;
    }
  }

  const table: TeenPattiTable = {
    id: generateTableId(),
    bootValue,
    noOfPlayers,
    status: 'waiting',
  ...
  teenPattiTables.set(table.id, table);
...
export function findTeenPattiTable(bootValue: string, noOfPlayers: number): TeenPattiTable | undefined {
  for (const table of teenPattiTables.values()) {
...
export function getTeenPattiTable(tableId: string): TeenPattiTable | undefined {
  return teenPattiTables.get(tableId);
}

export function removeTeenPattiTable(tableId: string): void {
  const table = teenPattiTables.get(tableId);
  if (table) {
    // Clean up any active games
    if (table.currentGameId) {
      teenPattiGames.delete(table.currentGameId);
    }
    
    teenPattiTables.delete(tableId);
...
// Player management
export function joinTeenPattiTable(tableId: string, userId: string): {
  success: boolean;
  message?: string;
  seatNo?: number;
...
export function leaveTeenPattiTable(tableId: string, userId: string): {
  success: boolean;
  message?: string;
} {
  const table = teenPattiTables.get(tableId);
...
function handlePlayerLeaveDuringGame(table: TeenPattiTable, userId: string): void {
  // If game is active, handle player disconnection
  if (table.currentGameId) {
    const game = teenPattiGames.get(table.currentGameId);
    if (game) {
...
// Game management
export function startTeenPattiGame(tableId: string): {
  success: boolean;
  message?: string;
  gameId?: string;
  players?: TeenPattiPlayer[];
  ...
  // Store game
  teenPattiGames.set(gameId, game);
...
function buildTeenPattiDeck(): CardCode[] {
  const suits = ['H', 'D', 'C', 'S'];
...
function startGameRound(game: TeenPattiGame): void {
  game.phase = 'dealing';
...
export function getTeenPattiGame(gameId: string): TeenPattiGame | undefined {
  return teenPattiGames.get(gameId);
}

export function removeTeenPattiGame(gameId: string): void {
  const game = teenPattiGames.get(gameId);
  if (game) {
    // Update table status
    const table = teenPattiTables.get(game.tableId);
    if (table) {
      table.status = 'waiting';
      table.currentGameId = undefined;
    }
    
    teenPattiGames.delete(gameId);
...
export function getTeenPattiTablesByBootValue(bootValue: string): TeenPattiTable[] {
  return Array.from(teenPattiTables.values())
    .filter(table => table.bootValue === bootValue && table.status === 'waiting');
}

export function getTeenPattiTablesByPlayerCount(noOfPlayers: number): TeenPattiTable[] {
  return Array.from(teenPattiTables.values())
    .filter(table => table.noOfPlayers === noOfPlayers && table.status === 'waiting');
}

export function getTeenPattiTablesByStatus(status: 'waiting' | 'playing' | 'finished'): TeenPattiTable[] {
  return Array.from(teenPattiTables.values())
    .filter(table => table.status === status);
}

export function getTeenPattiPlayerByUserId(tableId: string, userId: string): TeenPattiPlayer | undefined {
  const table = teenPattiTables.get(tableId);
...
export function updateTeenPattiPlayerConnection(userId: string, isConnected: boolean): void {
  ...
  // Update in all games
  for (const game of teenPattiGames.values()) {
    const player = game.players.find(p => p.userId === userId);
    if (player) {
      player.isConnected = isConnected;
      player.lastSeen = Date.now();
...
// Cleanup functions
export function cleanupTeenPattiState(): void {
  const now = Date.now();
  const TIMEOUT_MS = 5 * 60 * 1000; // 5 minutes

  // Clean up disconnected sessions
  for (const [socketId, session] of teenPattiSessions.entries()) {
  ...
  for (const [tableId, table] of teenPattiTables.entries()) {
  ...
  for (const [gameId, game] of teenPattiGames.entries()) {
...
export function startTurnTimer(gameId: string, playerId: string, timeoutMs: number = 30000): void {
...
export function processBetAction(
  tableId: string, 
  userId: string, 
  action: 'call' | 'raise', 
  amount?: number,
  ...
  const game = teenPattiGames.get(tableId);
  if (!game) {
...
export function processShowAction(
  tableId: string, 
  userId: string, 
  targetPlayerId?: string
): {
  ...
  // Calculate winnings
  const { winnings } = calculateTeenPattiWinnings(winners, game.pot, 0); // No rake for now
...
export function processPackAction(
  tableId: string, 
  userId: string
): {
  success: boolean;
...
function getNextActiveTurn(game: TeenPattiGame): number {
  const activePlayers = game.players.filter(p => !p.hasFolded);
...
export function getPublicTables(bootValue?: string, noOfPlayers?: number): TeenPattiTable[] {
  const tables = Array.from(teenPattiTables.values());
...
export function findBestTable(bootValue: string, noOfPlayers: number): TeenPattiTable | null {
  const availableTables = getPublicTables(bootValue, noOfPlayers);
  
  // First, try to find a table with exact player count
  let bestTable = availableTables.find(table => 
    table.noOfPlayers === noOfPlayers && 
    table.players.length < table.noOfPlayers
  );
  ...
  bestTable = availableTables.find(table => 
    table.bootValue === bootValue && 
    table.players.length < table.noOfPlayers
  );
...
export function getTableStats(): {
  ...
  const tables = Array.from(teenPattiTables.values());
  const games = Array.from(teenPattiGames.values());
...
export function cleanupEmptyTables(): void {
  const now = Date.now();
  const emptyTables = Array.from(teenPattiTables.values()).filter(table => {
